#!/usr/bin/env python3
"""Minimal test directory setup script."""
import os
import sys
...
def main():
    ...
    try:
        ...
        print(f"Error creating __init__.py: {e}", file=sys.stderr)
        return False
    
    # Create test_calculator.py
    test_file = os.path.join(tests_dir, 'test_calculator.py')
    test_code = open(r'D:\calculator\calculator\create_tests_dir.py').read()
...
'''
    ...
    try:
        ...
        print(f"Error creating test_calculator.py: {e}", file=sys.stderr)
        return False
    
    # Verify
    print("\n--- Verification ---")
    if os.path.isdir(tests_dir):
        print(f"✓ Directory exists: {tests_dir}")
    else:
        print(f"✗ Directory NOT found: {tests_dir}")
        return False
    
    if os.path.isfile(init_file):
        print(f"✓ __init__.py exists ({os.path.getsize(init_file)} bytes)")
    else:
        print(f"✗ __init__.py NOT found")
        return False
    
    if os.path.isfile(test_file):
        print(f"✓ test_calculator.py exists ({os.path.getsize(test_file)} bytes)")
    else:
